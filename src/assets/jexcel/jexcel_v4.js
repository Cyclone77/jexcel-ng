...
;(function (global, factory) {
    ...
    var jexcel = (function(el, options) {
                ...
                for (var i = px; i <= ux; i++) {
                    for (var j = py; j <= uy; j++) {
                        if (obj.records[j][i] && obj.records[j][i].getAttribute('data-merged')) {
                            ...
                            // 选中合并行和列
                            if (obj.options.isSelectedMerge) {
                                if (colspan > 1) {
                                    if (x < px) {
                                        px = x;
                                    }
        ...
        obj.updateCornerPosition = function() {
            ...
            if (! obj.highlighted.length) {
                ...
                if (obj.options.freezeRows) {
                    var height = obj.getFreezeheight();
                    if (y2 - y1 + h2 < width) {
                        obj.corner.style.display = 'none';
                    } else {
                        obj.corner.style.display = '';
        ...
        obj.scrollControls = function(e) {
            ...
            if (obj.options.freezeRows > 0 && obj.content.scrollTop != scrollTop) {
                obj.updateFreezeHeight();
            }
        ...
        obj.getFreezeheight = function () {
            var height = 0;
            if (obj.options.freezeRows > 0) {
                for (var i = 0; i < obj.options.freezeRows; i++) {
                    height += parseInt(obj.options.rows[i].height);
                }
            }
            return height;
        }
        ...
        obj.updateFreezeHeight = function () {
            scrollTop = obj.content.scrollTop;
            var height = 0;
            if (scrollTop > 27) {
                for (var i = 0; i < obj.options.freezeRows; i++) {
                    if (i > 0) {
                        height += parseInt(obj.options.rows[i - 1].height);
                    }
                    
                    [].forEach.call(obj.rows[i].children, (td, index) => {
                        if ([].indexOf.call(td.classList, 'jexcel_row') === -1) {
                            td.classList.add('jexcel_freezed_row');
                            td.style.top = height + 'px';
                        }
                        if (index <= obj.options.freezeColumns) {
                            td.classList.add('jexcel_freezed_common');
                        }
                    })
                    for (var j = 0; j < obj.colgroup.length; j++) {
                        if (obj.colgroup[j] && obj.records[i][j]) {
                            var shifted = (scrollTop + (i > 0 ? obj.records[i-1][j].style.height : 0)) - 28 + 'px';
                            obj.records[i][j].classList.add('jexcel_freezed_row');
                            obj.records[i][j].style.top = shifted;
                        }
                    }
                }
            } else {
                for (var i = 0; i < obj.options.freezeRows; i++) {
                    [].forEach.call(obj.rows[i].children, td => {
                        td.classList.remove('jexcel_freezed_row');
                        td.style.top = '';
                    })
                }
            }
